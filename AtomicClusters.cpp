...
int read_element_top(MPI_Win win_q, MPI_Win win_offs, int * element, int target_mpitask_id, int num_tries) // queue_del
{
    ...
    while(free == 0 && (num_tries==0 || tries_cntr<=num_tries)) //try to lock offs window putting -2 value
    {
        t0=MPI_Wtime();
    ...
    if(free == 0) // q is still blocked - go furter
    {
        ret = 2;
        return(ret);
    }
    ...
    {
        t0=MPI_Wtime();
        MPI_Win_lock( MPI_LOCK_EXCLUSIVE, target_mpitask_id, 0, win_q ); //lock the q
...
int add_element_unsorted(MPI_Win win_q, MPI_Win win_offs, int element[], int mpitask_id) //add_new_particle_to_queue //adds element to the end of the q
{
    int free = 0;
    int my_offset;
    double t0;

    while(free == 0) //try to lock offs window putting -2 value
    {
        t0=MPI_Wtime();
        MPI_Win_lock( MPI_LOCK_EXCLUSIVE, mpitask_id, 0, win_offs );
    ...
    t0=MPI_Wtime();
    MPI_Win_lock( MPI_LOCK_EXCLUSIVE, mpitask_id, 0, win_q ); //lock the q
    MPI_Put( element, 4, MPI_INT, mpitask_id, my_offset, 4, MPI_INT, win_q ); //put new val to the end of the q
    MPI_Win_unlock( mpitask_id, win_q );
...
int main(int argc, char * argv[])
{
	...
	/* deifining variables */
	int mpitask_id, number_of_processors, no_of_cpu_allocated, temp, atomic_number, processors;
	int wall_time, c, tflag = 0, aflag = 0, nflag = 0, zflag = 0;

	int NMZ_MIN, NMZ_MAX, NB_MIN, NB_MAX, T_MIN, T_MAX, A_0_MIN, A_0_MAX, WALL_TIME, T_DISP, A_0_DISP, NB_DISP, NMZ_DISP;
	int NMZ_SIZE, NB_SIZE, T_SIZE, A_0_SIZE; //range of values of NMZ , NB, T, A_0
	...
	data3 table;
	table.total_max_scal = 0;
	table.total_exec_time = 0.;
	table.min_wall_time = 0.;
	table.max_no_of_procs = 0;
	...
	NMZ_DISP = 1;
	cswitch = 0;
	debug = 0;//atoi(argv[11]);


	static struct option long_options[] = {
		{ "da", required_argument, 0, 'D' },
		{ "dt", required_argument, 0, 'd' },
		{ "dn", required_argument, 0, 'b' },
		{ "dz", required_argument, 0, 'm' },
		{ 0, 0, 0, 0 }
	};
	int long_index = 0;
	while ((c = getopt_long_only(argc, argv, "t:T:a:A:n:N:z:Z:d:D:b:m:c:", long_options, &long_index)) != -1)
	{
		switch (c)
		{
		case 'c': ///Sets the coulomb force switch
			if (atoi(optarg) == 0)
			{
				cswitch = 0;
			}
		...
		case 'b':
			...
			break;
		case 'm':
			NMZ_DISP = atoi(optarg);
			break;
		case 'D':
			A_0_DISP = atoi(optarg);
			break;
		case 'd':
			T_DISP = atoi(optarg);
			break;
    ...
    my_queue = (int *)calloc(WINDOW_SIZE,disp_unit);
	...
	{
		A_0_DISP = 1;
		/*printf("A_0_DISP cannot be zero!!!");
		MPI_Finalize();
		return 0;*/
	}
	...
	int a_size, t_size, j, k = 0, nb_size, nmz_size;

	char foldername[50];

	if (mpitask_id == 0)
	{
		for (j = 0; j<T_SIZE; j += T_DISP)
		{
			sprintf(foldername, "T_%d", j + T_MIN);
			mkdir(foldername, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
		}
	}

	//printf("Initialise the Table\n");
	j = 0;
	for (k = 0; k<A_0_SIZE; k += A_0_DISP)
	{
		table.a_no[j++] = (A_0_MIN + k);
		//printf("%d\t%d\n",table.temperature[t_size],table.adata[t_size].a_no[a_size]);
	}
	double a = T_SIZE / T_DISP, b = A_0_SIZE / A_0_DISP, e = NB_SIZE / NB_DISP, d = NMZ_SIZE / NMZ_DISP;
	t_size = ceil(a);
	a_size = ceil(b);
	nb_size = ceil(e);
	nmz_size = ceil(d);
	...
	//printf("Read data from File into the Table\n");

    if(rank==0){
        int start= A_0_MIN;
        while(start <= A_0_MAX){

		sprintf(mytimefile, "mytime_%d.txt", start);
		MPI_File_open(MPI_COMM_SELF, mytimefile, (MPI_MODE_WRONLY | MPI_MODE_CREATE | MPI_MODE_APPEND), MPI_INFO_NULL, &fh);
		MPI_File_close(&fh);

		sprintf(totaltimefile, "totaltime_%d.txt", start);
		MPI_File_open(MPI_COMM_SELF, totaltimefile, (MPI_MODE_WRONLY | MPI_MODE_CREATE | MPI_MODE_APPEND), MPI_INFO_NULL, &fh1);
        MPI_File_close(&fh1);
	...
	MPI_Comm_dup(MPI_COMM_WORLD, &new_comm);
		...
		//int i = mpitask_id;
			...
			//int job[4], i, NMZ, NB, T, A_0, MAX_SIZE;

    vec = w1.worker(mpitask_id, NMZ_MIN, NMZ_MAX, NB_MIN, NB_MAX, T_MIN, T_MAX, T_DISP, A_0_DISP, NB_DISP, NMZ_DISP, A_0_MIN, A_0_MAX);

    ftime = MPI_Wtime();

    /*
    vec.push_back(mpitask_id);
    vec.push_back(totaljobs_inq);
    vec.push_back(gen_q_time);
    vec.push_back(work_own_q_time);
    vec.push_back(counter);
    vec.push_back(stealing_time);
    vec.push_back(effective);
...
*/
    ...
    MPI_File_write_at(fh,(mpitask_id+1)*200,timefile,writecount,MPI_CHAR,MPI_STATUS_IGNORE);
...
int Solve_rmf::get_self_energies(int ic)
{
    // Objects Decleration
    Get_mf gm;
    Get_be_cl3 gbc1, gbc2;
...
int Solve_rmf::solve_rmf(int iph, double r)
{
    // Objects Declaration
    Get_chemical_potential_bar gcpb;
    Get_chemical_potential_lep gcpl;
    Get_properties gp;
    Cycle_solve cs;
    ...
    if ((IN_PART[2] == 1) || (IN_PART[3] == 1) || (IN_PART[4] == 1))
        MU_LL = gcpl.get_chemical_potential_lep();
    /*printf(" MU_LL %e\n", MU_LL*HBARC);*/
    ...
    gcpb.get_chemical_potential_bar(mu_b, mu_q);
    mu_b = F_MU_B;
    mu_q = F_MU_Q;
    ...
    ic3 = 0;
    /*************************/
    if ((int)cs.cycle_solve(iph, mu_b, mu_q) == 999)
        return 999;
    /*************************/
    /*mu_b = F_MU_B;
    mu_q = F_MU_Q;*/
...
int Solve_rmf::init_discr_ws_cell(int iph)
{
...
double Solve_rmf::f_ws(double r, double rr, double a)
{
    double tmp;
...
int Solve_rmf::init_self_energies(void)
{
    int ir, ip;
...
int Solve_rmf::get_anz_nuc(void)
{
...
int Cycle_solve::cycle_solve(int iph, double mu_b, double mu_q)
{
    Get_chemical_potential_bar gcpb1, gcpb2;
    Get_chemical_potential_lep gcpl1, gcpl2;
    Solve_rmf sr,sr1;
    ...
    if (IPHASE > -1)
    {
        ...
        do
        {
            ...
            mu_b_old = mu_b;
            mu_q_old = mu_q;
            sr.get_self_energies(ic);

            if ((IN_PART[2] == 1) || (IN_PART[3] == 1) || (IN_PART[4] == 1))
                MU_LL = gcpl1.get_chemical_potential_lep();

            if (gcpb1.get_chemical_potential_bar(mu_b, mu_q) == 999) return 999;
            mu_b = F_MU_B;
            mu_q = F_MU_Q;
    ...
    */
    ...
    do
    {
        ...
        mu_b_old = mu_b;
        mu_q_old = mu_q;
        sr1.get_self_energies(ic);

        if ((IPHASE > 0) &&
            ((IN_PART[2] == 1) || (IN_PART[3] == 1) || (IN_PART[4] == 1)))
            MU_LL = gcpl2.get_chemical_potential_lep();

        gcpb2.get_chemical_potential_bar(mu_b, mu_q);
        mu_b = F_MU_B;
        mu_q = F_MU_Q;
...
int Cycle_solve::init_acc0(void)
{
...
double F_func::f_func(double k, int ifunc, double mu, int ipa)
{
...
double F_func::func_fermi(double k, double mu, int ipa)
{
    double tmp;
...
double F_func::func_bose(double k, double mu, int ipa)
{
    double tmp;
...
int Fit_radius2::fit_radius2(int iph, double r_in)
{
    int iwr;
...
double Fit_radius2::solve_a(int ic, int iph, double a)
{
    Solve_rmf sr1, sr2;
    int iwr;
    double r;
    ...
    /*printf(" a r %e %e\n", a, r);*/
    if (IMFS[iph] == 0) XCL = 0;
    if ((int)sr1.solve_rmf(iph, r) == 999) return 999;
    IMFS[iph] = 1;
    XCL = 1;
    if ((int)sr2.solve_rmf(iph, r) == 999) return 999;
    /*IMFS[iph] = 0;*/
    if (iwr == 1)
    {
...
int Fit_radius2::fit_aa4_new(int iph, double r)
{
...
int Fold_dens::fold_dens(int ip)
{
...
double Fold_dens::func_fold(double r, double rp, double a2)
{
    double tmp;
...
int Get_be_cl3::get_be_cl3(int i1, double n_p, double n_n)
{
...
int Get_be_cl3::get_dbe_p_g(int icl)
{
    double tmp0, tmp1;
...
int Get_be_cl3::get_dbe_p_j(void)
{
...
int Get_be_cl3::get_shift(double de, double det, double x, double x0)
{
...
/*************************************************************************************************************/
Get_chemical_potential_bar::Get_chemical_potential_bar()
{
}
...
int Get_chemical_potential_bar::get_chemical_potential_bar(double mu_b, double mu_q)
{
    double dmu_q, dmu_b;
    int ip;
    ...
    if (ICOUNT == 3)
    {
        ...
        //exit(0);
        printf("get_chemical_potential_bar Exit()");
        return 0;
    }
...
int Get_chemical_potential_bar::fit_asymmetry(double mu_b, double dmu_b, double mu_q, double dmu_q)
{
...
int Get_chemical_potential_bar::fit_density(double mu_b, double dmu_b, double mu_q)
{
...
int Get_chemical_potential_bar::get_nb_yq(double mu_b, double mu_q, int ic)
{
    //Objects Declaration
    Get_rho_part grp1, grp2, grp3, grp4;
    ...
    {
        mu = PARTICLE[ip].mu;
        rho = grp1.get_rho_part(mu, ip, ic);
    ...
    if (XCL == 1)
    {
        for (i1 = 0; i1<N_CL; i1++)
        {
            ...
            if (IN_PART[ip] == 1)
            {
                ...
                rho = grp2.get_rho_part(mu, ip, ic);
                /*printf("d %f %e\n", mu*HBARC,rho);*/
        ...
        if (IN_HYP == 1)
        {
            for (i1 = 0; i1<N_HYP; i1++)
            {
                ...
                if (IN_PART[ip] == 1)
                {
                    ...
                    rho = grp3.get_rho_part(mu, ip, ic);
                    tmpb += rho*(double)PARTICLE[ip].a;
                    tmpq += rho*(double)PARTICLE[ip].z;
                }
            }
    ...
    if (IN_TMES == 1)
    {
        for (i1 = 0; i1<N_TMES; i1++)
        {
            ...
            if (IN_PART[ip] == 1)
            {
                mu = PARTICLE[ip].mu = mu_q*(double)PARTICLE[ip].z
                    + mu_b*(double)PARTICLE[ip].a;
                rho = grp4.get_rho_part(mu, ip, ic);
                tmpb += rho*(double)PARTICLE[ip].a;
                tmpq += rho*(double)PARTICLE[ip].z;
            }
        }
...
int Get_chemical_potential_bar::get_dens_ton(int ic)
{
    int ip, iq;
...
int Get_chemical_potential_bar::heapsort(int n)
{
...
/*************************************************************************************************************/
Get_chemical_potential_lep::Get_chemical_potential_lep()
{
}
...
double Get_chemical_potential_lep::get_chemical_potential_lep(void)
{
...
double Get_chemical_potential_lep::get_n_lep(double mu, double fc, int ic)
{
    // Objects Declaration
    Get_rho_part grp;
...
int Get_eos_table::get_eos_table(int inse, int in_g, int in_e, int in_mu, int in_tau, int in_nu,
...
int Get_eos_table::get_parameter_tables(int ir_y, int ir_n, int ir_t,
...
double Get_eos_table::get_eos_local_beta(double t, double n_b, int inse,
...
int Get_nucleus::get_nucleus(double a, double z, char *cnsfile)
{
    // Objects Declaration
    Solve_rmf sr1, sr2, sr3;
    Get_nuc_table gnt;
    ...
    {
        if ((int)sr3.solve_rmf(iph, r) == 999) return 999;
    }
    ...
    */
    ...
    gnt.get_results_ws_cell(cnsfile);

    return 0;
}
/*****************************************************************************/

int Get_nucleus::init_sol(void)
{
    int iph;
...
int Get_nucleus::iexs_sol(int iph)
{
    double tmp;
...
int Get_nucleus::save_sol(int iph)
{
    int ip, ir, iq, imf;
...
/*****************************************************************************/
/*************************************************************************************************************/
Get_eos_composition::Get_eos_composition()
{
}
...
int Get_eos_composition::get_eos_composition(int in_cell, char* cnsfile)
{
    // Objects Declaration
    Get_nucleus gn1, gn2, gn3;
    Solve_rmf sr1, sr2;
    Get_nuc_table gnt;
    Fold_dens fd1,fd2;
    Fit_radius2 fr;
    ...
    if (IN_PH[iph] == 1)
    {
        ...
        r = 0.;
        XCL = 0;
        if ((int)sr1.solve_rmf(iph, r) == 999)
            return 999;
        gn1.save_sol(iph);
    ...
    */
    ...
    /*printf(" %i %i\n", iph, IMFS[iph]);*/
    /*IMFS[iph] = 0;*/
    if ((int)sr2.solve_rmf(iph, r) == 999)
        return 999;
    ...
    if (IPHASE > 0)
    {
        if (ISOL > 0) gnt.get_results_ws_cell(cnsfile);
...
int Get_eos_composition::get_fractions(void)
{
...
int Get_eos_composition::get_coul_corr(void)
{
    int ip, ir;
...
int Get_eos_composition::select_sol(int ip_max)
{
...
int Get_mf::get_mf(int ic)
{
    ...
    poke_pe(); //poke the progress engine to check for waiing lock/unlock requests
    ...
    */
    ...
    poke_pe(); //poke the progress engine to check for waiting lock/unlock requests

    return 0;
}
/*****************************************************************************/

int Get_mf::get_mf_greens(int imf, double *tmps)
{
    int ir, irp, ix;
...
double Get_mf::simpson_mod(int np, double *vec)
{
    int ir, irp, n;
...
int Get_mf::init_acc_mf(int imf)
{
    int ip, ir, iq;
...
int Get_mf::get_ton_mass(double n_p, double n_n)
{
...
int Get_mf::get_ton_mass_shift(int ip, double n_ref, double n_eff)
{
    double tmp, f, df, fac;
...
int Get_mat_inv::lu_backsubstitution(int n)
{
...
int Get_mat_inv::lu_decomposition(int n)
{
...
int Get_mat_inv::get_mat_inv(int dim)
{
    int i1, i2;
...
double Get_k2_red::get_k0_red(double x)
{
    double tmp, q, i0, t2;
...
double Get_k2_red::get_k1_red(double x)
{
    double tmp, q, i1, t2;
...
double Get_k2_red::get_k2_red(double x)
{
    double tmp;
...
int Get_properties::get_properties(void)
{
    // Objects Declaration
    Get_be_cl3 gbc1, gbc2, gbc3, gbc4;
...
int Get_properties::get_prop_part(int ip)
{
...
int Get_properties::get_prop_ton(void)
{
    int ip, iq;
...
int Get_properties::get_prop(double mu)
{
    // Objects Declaration
    Get_rho_part grp1, grp2, grp3, grp4;
    Get_k2_red gkr1,gkr2;
    ...
    if (TT > 0.)
    {
        ...
        case -1:   /* Bose-Einstein, particle = antiparticle */
        {
            ...
            IBX = ipa = 0;
            grp1.get_simint_bounds_b(mup, ipa);
            F_ENE += fac*grp2.get_simint(10, 1.e-08, 14, F_KM, F_KP, mup, ipa);
            F_ENT -= fac*grp3.get_simint(10, 1.e-08, 16, F_KM, F_KP, mup, ipa);
            F_PRE += fac*grp4.get_simint(10, 1.e-08, 15, F_KM, F_KP, mup, ipa) / 3.;
            break;
        }
        ...
        case 3:   /* Maxwell-Boltzmann, relativistic */
        {
            ...
            tmpn = tmp2*gkr1.get_k2_red(fabs(F_MD) / TT);
            tmpns = tmp2*gkr1.get_k1_red(fabs(F_MD) / TT);
            F_ENE = 3.*TT*tmpn + fabs(F_MD)*tmpns;
            F_ENT = 4.*tmpn + (fabs(F_MD)*tmpns - (mu - F_V)*tmpn) / TT;
            F_PRE = tmpn*TT;
            tmp2 = tmp1*exp((F_V - fabs(F_MD) - mu) / TT);
            tmpn = tmp2*gkr2.get_k2_red(fabs(F_MD) / TT);
            tmpns = tmp2*gkr2.get_k1_red(fabs(F_MD) / TT);
            F_ENE += 3.*TT*tmpn + fabs(F_MD)*tmpns;
            F_ENT += 4.*tmpn + (fabs(F_MD)*tmpns + (mu - F_V)*tmpn) / TT;
            F_PRE += tmpn*TT;
            break;
        ...
        default:   /* Fermi-Dirac */
        {
            for (ipa = 0; ipa<2; ipa++)
            {
                grp1.get_simint_bounds_f(mu, ipa);
                if (F_KP > F_KM)
                {
                    ene = grp2.get_simint(10, 1.e-08, 4, F_KM, F_KP, mu, ipa);
                    ent = grp3.get_simint(10, 1.e-08, 6, F_KM, F_KP, mu, ipa);
                    pre = grp4.get_simint(10, 1.e-08, 5, F_KM, F_KP, mu, ipa) / 3.;
                }
                else
                {
                    ene = ent = pre = 0.;
...
data2* Get_nuc_table::get_nuc_table(int inse, int in_g, int in_e, int in_mu, int in_tau, int in_nu,
...
int Get_nuc_table::get_eos_local(double t, double n_b, double y_q, int inse, int in_g, int in_e, int in_mu, int in_tau, int in_nu, int *in_cl, int in_hyp, int in_tmes, int in_cell, int in_ton, int inuc, double a, double z, char *cnsfile)
{
    // Objects Declaration
    Get_eos_composition gec;
    Get_nucleus gn;
    ...
    if (inuc == 1)
    {
        ...
        IN_TMES = 0;
        for (ip = 0; ip<N_TMES; ip++) IN_PART[9 + N_CL + N_HYP + ip] = 0;
        if ((int)gn.get_nucleus(a, z, cnsfile) == 999) return 999;
        /*printf(" ICV_RMF = %i\n", ICV_RMF);*/
    ...
    else
    {
        ...
        {
            if ((int)gec.get_eos_composition(in_cell, cnsfile) == 999)
                return 999;
            /*get_properties();*/
            /*get_fractions();*/
        }
...
int Get_nuc_table::get_results_ws_cell(char *cnsfile)
{
    int ir;
    FILE *file_dat;
...
double Get_rho_part::get_rho_part(double mu, int ip, int ic)
{
...
int Get_rho_part::get_rho(double mu)
{
    //Objects Declaration
    Get_k2_red gkr;
    ...
    if (TT > 0.)
    {
        ...
        case 3:   /* Maxwell-Boltzmann, relativistic */
        {
            ...
            F_RHOP = F_G*exp((mu - fabs(F_MD) - F_V) / TT)*tmp;
            F_RHOA = F_G*exp((F_V - fabs(F_MD) - mu) / TT)*tmp;
            tmp = gkr.get_k2_red(fabs(F_MD) / TT);
            F_RHOP *= tmp;
            F_RHOA *= tmp;
            break;
        }
        case 1:   /* Fermi-Dirac */
        {
            ...
            if (tmp2 < -10.)
            {
                tmp = fabs(F_MD) / TT;
                tmp2 = exp(tmp2);
                F_RHOP = tmp2*gkr.get_k2_red(tmp);
                F_RHOP -= QUAD(tmp2)*gkr.get_k2_red(2.*tmp) / 2.828427125;
                tmp = sqrt(fabs(F_MD)*TT / TPI);
                tmp = F_G*CUBE(tmp);
                F_RHOP *= tmp;
            }
            ...
            if (tmp2 < -10.)
            {
                tmp = fabs(F_MD) / TT;
                tmp2 = exp(tmp2);
                F_RHOA = tmp2*gkr.get_k2_red(tmp);
                F_RHOA -= QUAD(tmp2)*gkr.get_k2_red(2.*tmp) / 2.828427125;
                tmp = sqrt(fabs(F_MD)*TT / TPI);
                tmp = F_G*CUBE(tmp);
                F_RHOA *= tmp;
            }
...
int Get_rho_part::get_rhos(double mu)
{
    // Objects Declaration
    Get_k2_red gkr;
    ...
    if (TT > 0.)
    {
        ...
        case 3:   /* Maxwell-Boltzmann, relativistic */
        {
            ...
            F_RHOP = F_G*exp((mu - fabs(F_MD) - F_V) / TT)*tmp;
            F_RHOA = F_G*exp((F_V - fabs(F_MD) - mu) / TT)*tmp;
            tmp = gkr.get_k1_red(fabs(F_MD) / TT);
            F_RHOP *= tmp;
            F_RHOA *= tmp;
            break;
        }
...
double Get_rho_part::get_simint(int order_max, double prec, int ifunc,
    double x_min, double x_max, double rpar, int ipar)
{
    // Objects Declaration
    F_func ff1, ff2, ff3, ff4;
    ...
    dx = x_max - x_min;
    res = ff1.f_func(x_min, ifunc, rpar, ipar)
        + ff2.f_func(x_max, ifunc, rpar, ipar);
    ...
    if (order_max > 0)
    {
        ...
        ip = 1;
        x = x_min + 0.5*dx;
        tmp1 = ff3.f_func(x, ifunc, rpar, ipar);
        tmp2 = res + 4.*tmp1;
        tmp3 = tmp2*dx / 6.;
        err = fabs(tmp3);
        for (io = 1; io<io_max; io++)
        {
            if ((io < 4) || (err > prec*tmp3))
            {
                ...
                for (ip = 0; ip<ip_max; ip++)
                {
                    ...
                    tmpx = ff4.f_func(x, ifunc, rpar, ipar);
                    tmp1 += tmpx;
                }
                tmp2 = res + 4.*tmp1;
...
int Get_rho_part::get_simint_bounds_f(double mu, int ipa)
{
    double em, ep, tmp;
...
int Get_rho_part::get_simint_bounds_b(double mu, int ipa)
{
    double em, ep, tmp;
...
double Get_rho_part::inv_disp_rel(double e, int ipa)
{
    double ml, k, tmp;
...
double Get_rho_part::disp_rel(double k, int ipa)
{
    double tmp;
...
int Init_eos::init_eos(int ipara, int imb, int in_ton, char*cnsfile)
{
...
int Init_eos::init_parameters(void)
{
    ...
    {
        ALPHA = 1. / 137.035999679; // Coloumb force switch
    }
    E2 = ALPHA*HBARC;
    G_G = sqrt(FPI*ALPHA);
    AMU = 931.494028;
...
int Init_eos::init_rmf(int ipara)
{
    int imf;
...
int Init_eos::init_nuclei_ame03(int in_ton, char *cnsfile)
{
...
int Init_eos::init_particles(int imb)
{
    int ip, icl;
...
int Init_eos::init_ton(int in_ton, char *cnsfile)
{
...
int Init_be_cl::init_be_cl(void)
{
...
double Init_be_cl::experfc(double z)
{
    double z2, z4, z6, res;
    ...
    {
        printf(" error in subroutine experfc ");
        // exit(0);
        return 0;
    }
...
data2* Worker::work(int task[], int size,int mpitask_id,int displacement,double *worked_for,int displacement1)
{
	int writer_orig_rank;
	int old_mpitask_id;

	MPI_Comm_rank(MPI_COMM_WORLD, &old_mpitask_id);
	...
	double mytime, initime;

	MPI_Comm_size(MPI_COMM_WORLD, &number_of_processors);

	int job[4];
	for (j = 0; j<4; j++) ///Takes values for the first job from
		job[j] = task[j];
	...
	int ipara, inuc, imb, inse, in_g, in_e, in_mu, in_tau, in_nu, in_cl[N_CL], in_hyp, in_tmes, in_ton, in_phase[N_PH], in_r, in_tab;
    ...
    *worked_for = mytime;
    ...
    offset = displacement * sizeof(char) * 35;
    sprintf(mytimefile, "mytime_%d.txt", A_0);
	write_count = sprintf(time, "\n%d %d %d %d %lf %d %d\t", NMZ, T, NB, A_0, mytime, mpitask_id, old_mpitask_id);

	MPI_File_open(MPI_COMM_SELF, mytimefile, (MPI_MODE_WRONLY | MPI_MODE_CREATE | MPI_MODE_APPEND), MPI_INFO_NULL, &fh);


	//printf("%s:\t%ld\n",mytimefile,offset_a_0 );
	MPI_File_write_at(fh, offset, time, write_count, MPI_CHAR, MPI_STATUS_IGNORE);
	MPI_File_close(&fh);


	/*
	FILE *sakfile;
	sakfile=fopen(mytimefile,"a+");
	fprintf(sakfile,"%d %d %d %d %lf %d\n",NMZ,T,NB,A_0,mytime,mpitask_id);
	fclose(sakfile);
	*/

	if (count != 0) ///Write data stored in the 'point'
	{
		for (loopcount = 0; loopcount < count; loopcount++)
		{
			...
			char tstring[150];
			char atfile[50];
			int  write_count = sprintf(tstring, "\n%e %e %i %i %e %e %e %e %e %e %e %e\t", point->a[loopcount], point->b[loopcount], point->c[loopcount], point->d[loopcount], point->e[loopcount], point->f[loopcount], point->g[loopcount], point->h[loopcount], point->i[loopcount], point->j[loopcount], point->k[loopcount], point->l[loopcount]);

			//  sprintf(nucfile,"T_%f.dat", point->a[loopcount]);

			if (old_a0 != point->d[loopcount])
			{
			...
			//printf("%e %e %i %i %e %e %e %e %e %e %e %e", point->a[loopcount],point->b[loopcount],point->c[loopcount],point->d[loopcount],point->e[loopcount],point->f[loopcount],point->g[loopcount],point->h[loopcount],point->i[loopcount],point->j[loopcount],point->k[loopcount],point->l[loopcount]);

            offset_a_0 = displacement1 * sizeof(char) * 140;

			MPI_File_open(MPI_COMM_SELF, atfile, (MPI_MODE_WRONLY | MPI_MODE_CREATE), MPI_INFO_NULL, &fh1);


			//printf("Atomic_Number:\t%ld\n",offset_a_0 );

			MPI_File_write_at(fh1, offset_a_0, tstring, write_count, MPI_CHAR, MPI_STATUS_IGNORE);

			MPI_File_close(&fh1);

			offset_a_0 += sizeof(char) * 140;
		}
...
std::vector <double> Worker::worker(int mpitask_id, int NMZ_MIN, int NMZ_MAX, int NB_MIN, int NB_MAX, int T_MIN, int T_MAX, int T_DISP, int A_0_DISP, int NB_DISP, int NMZ_DISP, int A_0_MIN, int A_0_MAX)
{
	...
	int bufsize, bsize;
    int NMZ_SIZE, NB_SIZE, T_SIZE, A_0_SIZE;
    int nmz_size,nb_size,t_size,a_size;
    /*A_0 = job[3];
	T = job[2];
	NB = job[1];
	NMZ = job[0];
*/
	...
	MAX_SIZE = nb_size*nmz_size*a_size*t_size;
	...
	// snprintf(foldername,sizeof(foldername),"Writer_%d",writer_orig_rank);
	//
	//        mkdir(foldername,S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
	...
	/*MPI_Pack_size( 13, MPI_DOUBLE, new_comm, &s1 );
	bufsize = 177 * MPI_BSEND_OVERHEAD + s1*177;
	...
	*/
    ...
    a =  MAX_SIZE/number_of_processors;
    int task_split = ceil(a)+1;
    int i = mpitask_id*task_split;
    int counter=0;
	int end = task_split + i;
	if(mpitask_id == number_of_processors-1)
        end = MAX_SIZE;
    if(i>=MAX_SIZE)
        i = MAX_SIZE;
    if(end > MAX_SIZE)
    ...
    int totaljobs_inq=0;
    double gen_q_stime,gen_q_time=0;
    double work_own_q_stime,work_own_q_time=0;
    double stealing_stime,stealing_time;
    double effective;
    while(i<end)
    {
        // Produce jobs till either all jobs are produced or the queue is full
        my_offset = 0;
        gen_q_stime = MPI_Wtime();
        while (i<end && (my_offset<(WINDOW_SIZE-8)))
        {
            NMZ = (i%nmz_size)*NMZ_DISP + NMZ_MIN;
            NB = ((i / nmz_size) % nb_size)*NB_DISP + NB_MIN;
            T = (((i / nb_size) / nmz_size) % t_size)*T_DISP + T_MIN;
            A_0 = (((i / nb_size) / nmz_size) / t_size)*A_0_DISP + A_0_MIN;
            ...
            my_offset = add_element_unsorted(win_q, win_offs, job, mpitask_id);
            totaljobs_inq++;
        }
        gen_q_time += (MPI_Wtime()-gen_q_stime);

        //consume jobs from own queue


        empty=0;
        target_rank = rank; //SELF cycle
        work_own_q_stime = MPI_Wtime();
        while(empty == 0)
        {

            empty = read_element_top(win_q, win_offs, my_queue_element, rank, 0);
            ...
            if(empty == 0)
            {
                ...
                A_0 = my_queue_element[3];
                displacement = ((nb_size*nmz_size)*(T-T_MIN) + nmz_size*(NB-NB_MIN) + (NMZ-NMZ_MIN+1));
                displacement1 = nmz_size*(NB-NB_MIN) + (NMZ-NMZ_MIN+1);

                point = work(my_queue_element,4,mpitask_id,displacement,&worked_for,displacement1);
    ...
    effective = 0;
    stealing_stime = MPI_Wtime();

    //here goes the algorithm itself!!!!-------------------------------------------------------------------------------
    // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    // look for jobs from other processors
    ...
    while(accumulated_status>0)
    {
        ...
        for(cntr=0; (cntr<num_proc-1) && (empty==1); cntr++) // cycle: all processors;
        {
            ...
            if(target_rank!=rank)
            {
                ...
                if(status < 2) //not locked
                {
                    ...
                    if(empty == 0)
                    {
                        ...
                        A_0 = my_queue_element[3];
                        displacement = ((nb_size*nmz_size)*(T-T_MIN) + nmz_size*(NB-NB_MIN) + (NMZ-NMZ_MIN+1));
                        nmz_size*(NB-NB_MIN) + (NMZ-NMZ_MIN+1);

                        point = work(my_queue_element,4,mpitask_id,displacement,&worked_for,displacement1);
                        effective += worked_for;
                        mapping[A_0] += worked_for;
        ...
        if(empty == 1) // if nothing found - end of work
        {
    ...
    stealing_time = MPI_Wtime()-stealing_stime;
    vec.push_back(stealing_time);
    vec.push_back(effective);
    vec.push_back(stealing_time-effective);
    vec.push_back(counter);
	...
	if (count != 0)
	{
		...
		//free(point);
		offset_a_0 += (number_of_processors)*sizeof(char) * 140 * WRITE_BLOCK_SIZE;

		resetcount();
	}
	initime = MPI_Wtime();
	finaltime = initime - initialtime;
	char time[100];
	int write_count;

    if(mapping.empty() != 1){
    std:: map<int,double> :: iterator it = mapping.begin();
    do{
        sprintf(totaltimefile, "totaltime_%d.txt", it->first);

        write_count = sprintf(time, "\n%d\t%d\t%lf\t", mpitask_id, it->first, it->second);

        MPI_File_open(MPI_COMM_SELF, totaltimefile, (MPI_MODE_WRONLY | MPI_MODE_CREATE | MPI_MODE_APPEND), MPI_INFO_NULL, &fh1);

        MPI_File_write_at(fh1, sizeof(char) * 40 * (mpitask_id), time, write_count, MPI_CHAR, MPI_STATUS_IGNORE);
...
data2* Worker::myfunction(char *s, int t_nmz, int t_nb, int t_t, int a_0, data2*point)
{
    int ipara, inuc, imb, inse, in_g, in_e, in_mu, in_tau, in_nu,
        in_cl[N_CL], in_hyp, in_tmes, in_ton, in_phase[N_PH], in_r, in_tab;

    // Declaration of Objects for function calls
    Init_eos ie;
    Get_eos_table get;
    Get_nuc_table gnt;
    ...
    ie.init_eos(ipara, imb, in_ton, s);

    /***** CALCULATION *****/

    if (inuc == 1)
    {
        point = gnt.get_nuc_table(inse, in_g, in_e, in_mu, in_tau, in_nu, in_cl, in_hyp, in_tmes, t_nmz, t_nb, t_t, s, point, a_0);
    }
    else
    {
        get.get_eos_table(inse, in_g, in_e, in_mu, in_tau, in_nu, in_cl, in_hyp, in_tmes,
