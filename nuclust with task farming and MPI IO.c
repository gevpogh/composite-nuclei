...
data2* myfunction(char *,int,int,int,data2*);
...
/* nuclei table */
data2* get_nuc_table(int,int,int,int,int,int,int *,int,int,int,int,int,char *,data2 *);
...
/*****************************************************************************/
data2* myfunction(char *s,int t_nmz , int t_nb , int t_t, data2*point)
{
    ...
    if (inuc == 1)
    {
        point=get_nuc_table(inse,in_g,in_e,in_mu,in_tau,in_nu,in_cl,in_hyp,in_tmes,t_nmz,t_nb,t_t,s,point);
    }
    else
    {
...
data2* get_nuc_table(int inse,int in_g,int in_e,int in_mu,int in_tau,int in_nu,
                     int *in_cl,int in_hyp,int in_tmes,int t_nmz, int t_nb, int t_t, char *cnsfile,data2* point )
{
    int iwr,itmp,icl,iph,in_cell,inuc,in_a,nmz_min,nmz_max,aa_max,nmz,a_0,ia,
...
/*****************************************************************************/
int get_eos_local(double t,double n_b,double y_q,int inse,
                  int in_g,int in_e,int in_mu,int in_tau,int in_nu,
                  int *in_cl,int in_hyp,int in_tmes,
                  int in_cell,int in_ton,int inuc,double a,double z,char *cnsfile)
{
...
int get_chemical_potential_bar(double mu_b,double mu_q)
{
    ...
    dmu_b = 0.1;
    dmu_q = 0.05;
    fit_asymmetry(mu_b,dmu_b,mu_q,dmu_q);
...
main(int argc , char * argv[])
{
    ...
    int NMZ_MIN , NMZ_MAX , NB_MIN , NB_MAX , T_MIN , T_MAX;
    int NMZ_SIZE , NB_SIZE , T_SIZE; //range of values of NMZ , NB, T

    int NMZ , T , NB, i, job[3]; //values to be passed to the eos function known as myfunction()
    ...
    long int MAX_SIZE = (NMZ_SIZE)*(NB_SIZE)*(T_SIZE);
    /* MPI INITIALIZED */
...
void manager(int number_of_processors, int argc , char * argv[])
{
    int NMZ_MIN , NMZ_MAX , NB_MIN , NB_MAX , T_MIN , T_MAX;
    int NMZ_SIZE , NB_SIZE , T_SIZE; //range of values of NMZ , NB, T
    int NMZ , T , NB; //values to be passed to the eos function known as myfunction()

    int i=number_of_processors;
    /* ASSIGNMENT */
    T_MIN=atoi(argv[1]);
    T_MAX=atoi(argv[2]);
    NB_MIN=atoi(argv[3]);
    ...
    */
    while (i<(NMZ_SIZE)*(NB_SIZE)*(T_SIZE))
    {
        NMZ=(i%NMZ_SIZE)+NMZ_MIN;
        NB=((i/NMZ_SIZE)%NB_SIZE)+NB_MIN;
        T=((i/NB_SIZE)/NMZ_SIZE) + T_MIN;
        ...
        tag = status.MPI_TAG;
        MPI_Send(&job, 3, MPI_INT, who, i, MPI_COMM_WORLD);
        i++;
    }
    ...
    for (i=0; i<number_of_processors; i++)
    {
        ...
        who = status.MPI_SOURCE;
        tag = status.MPI_TAG;
        MPI_Send(&job, 3, MPI_INT, who, MAX_SIZE, MPI_COMM_WORLD);
    }
}

void worker (int mpitask_id, long int MAX_SIZE, int task[])
{
    int result, tag, j, reqcount,ab;
    int NMZ , T , NB;
    MPI_Status status;
    int loopcount=0;
...
//        myfile=fopen(changestring("J",constfile,".out"),"a+");
        ...
        point=myfunction(constfile,NMZ,NB,T,point);
        //FILE * tempfile;
        ...
        MPI_File_open(MPI_COMM_SELF,nucfile,(MPI_MODE_WRONLY | MPI_MODE_CREATE | MPI_MODE_APPEND|MPI_MODE_SEQUENTIAL), inf, &fh);

        for(loopcount=0; loopcount<count; loopcount++)
        {
            snprintf(tstring, 150 ,"%e %e %i %i %e %e %e %e %e %e %e %e", point->a[loopcount],point->b[loopcount],point->c[loopcount],point->d[loopcount],point->e[loopcount],point->f[loopcount],point->g[loopcount],point->h[loopcount],point->i[loopcount],point->j[loopcount],point->k[loopcount],point->l[loopcount]);

            snprintf(atfile,sizeof(atfile),"Atomic_Number_%i.dat", point->d[loopcount]);



            MPI_File_iwrite_shared( fh, tstring , sizeof(char)*150, MPI_CHAR, &req[loopcount]  );

            MPI_File_open(MPI_COMM_SELF,atfile,(MPI_MODE_WRONLY | MPI_MODE_CREATE | MPI_MODE_APPEND|MPI_MODE_SEQUENTIAL), inf, &fh1);


            MPI_File_iwrite_shared( fh1, tstring , sizeof(char)*150, MPI_CHAR, &req[loopcount]  );
        ...
        /*
                ...
                for(loopcount=0; loopcount<count; loopcount++)
                {
                    tempfile= fopen(nucfile,"a+");

                    fprintf(tempfile,"%e %e %i %i %e %e %e %e %e %e %e %e\n",point->a[loopcount],point->b[loopcount],
                            point->c[loopcount],point->d[loopcount],point->e[loopcount],point->f[loopcount],point->g[loopcount],
                            point->h[loopcount],point->i[loopcount],point->j[loopcount],point->k[loopcount],point->l[loopcount]);

                    fclose(tempfile);
                                snprintf(atfile,sizeof(atfile),"Atomic_Number_%i.dat", point->d[loopcount]);
                    tempfile= fopen(atfile,"a+");
        ...
        */
        free(point);
        // fprintf(tempfile,"count for temperature %d is %d\n",T,count);
        resetcount();
