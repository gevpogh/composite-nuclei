...
/*****************************************************************************/
        ...
        /***************************************/
        /** I have made the function knapnsort**/
        /** which first fills the available   **/
        /** Number of processors and then sort**/
        /** the remaining in the incresaing   **/
        /** order. This is not working for    **/
        /** some reason. Need to test and     **/
        /** find out the reason. Feel free to **/
        /** contact me at vishesh92@gmail.com **/
        /***************************************/

void knapnsort(data3 * table, data3 * new_table, int size, int cap)
{
    int i, alloc = 0,j = 0, k = 0,value, ptr;
    int flag[350];
    for(i = 0;i<size;i++)
    {
        flag[i] = 0;
    ...
    while(1)
    {
        if(alloc + table->max_scal[i] < cap)
        {
            alloc += table->max_scal[i];
            new_table->a_no[j] = table->a_no[i];
            new_table->max_scal[j] = table->max_scal[i];
            j++;
            flag[i] = 1;
        }
        i--;
    ...
    i = 0;
    int temp_scal, temp_a;
    while(j != size)
    {
        if(flag[i] == 0)
        {
            value = table->max_scal[i];
        }
        else
        {
            i++;
        ...
        for (k = i; k < size; k++)
        {
            if(flag[k] == 0)
            {
                if(value > table->max_scal[k])
                {
                    value = table->max_scal[k];
                    ptr = k;
                }
            }
        }
        if(i!= ptr)
        {
            temp_scal = table->max_scal[ptr];
            temp_a = table->a_no[ptr];
            table->max_scal[ptr] = table->max_scal[i];
            table->a_no[ptr] = table->a_no[i];
            table->max_scal[i] = temp_scal;
            table->a_no[i] = temp_a;
        }

        i++;
        if(i == size) printf("i == size");

        flag[ptr] = 1;
        new_table->a_no[j] = table->a_no[ptr];
        new_table->max_scal[j] = table->max_scal[ptr];
        j++;
    }
    return ;
}
...
/** Intialize MPI*/
    ...
    t_size = ceil(T_SIZE/T_DISP);
    a_size = ceil(A_0_SIZE/A_0_DISP);
    nb_size = ceil(NB_SIZE/NB_DISP);
    nmz_size = ceil(NMZ_SIZE/NMZ_DISP);
    ...
    if(data_file == NULL) ///Checks if file exists
    {
        printf("\n--------------------------------------------------------------------\n");
        printf("\nWARNING!!! 'data.dat' file not found. \n");
        printf("\n--------------------------------------------------------------------\n");
        printf("\nStarting the job with number of processors available.\n");
        printf("\n--------------------------------------------------------------------\n");
        ...
        if(mpitask_id == 0) manager(number_of_processors-1, NMZ_MIN , NMZ_MAX , NB_MIN , NB_MAX , T_MIN , T_MAX, T_DISP, A_0_DISP, NB_DISP, NMZ_DISP, A_0_MIN, A_0_MAX);
    ...
    {
        while(fscanf(data_file,"%d\t%d\n",&atomic_number,&processors)!=EOF)//Reading Temp Atomic_Number   Processors  Wall_Time from file. Assumed that the contents of the file are sorted
        {
            if(table.a_no[k] == atomic_number)
            {
                //table.min_time[k] = wall_time;
                table.max_scal[k] = processors;
                table.total_max_scal += processors;
                //if(table.min_wall_time < wall_time)table.min_wall_time = wall_time;
                if(table.max_no_of_procs < processors)table.max_no_of_procs = processors;
                k++;
                if(k == a_size)
                {
                    break;
    ...
    //printf("Table Completely Filled\n");

   /* if(table.min_wall_time > WALL_TIME)
    {
        printf("\n--------------------------------------------------------------------\n");
        printf("\nThe Time allocated (%d) is less than the time required (%lf). \n",WALL_TIME,table.min_wall_time);
        printf("\n--------------------------------------------------------------------\n");
        MPI_Finalize();
        return 0;
    }*/
    if(table.max_no_of_procs > number_of_processors )
    {
        printf("\n--------------------------------------------------------------------\n");
        printf("\nThe cores available (%d) is less than the minimum cores required (%d). \n",number_of_processors,table.max_no_of_procs);
        printf("\n--------------------------------------------------------------------\n");
        printf("\nStarting the job with number of processors available.\n");
        printf("\n--------------------------------------------------------------------\n");
    ...
    if(number_of_processors >= table.total_max_scal)
    {
        ///Use MPI_Comm_Split to create the groups and complete job
        int color = 0;
        no_of_cpu_allocated = 0;
        while(1)
        {
            no_of_cpu_allocated += table.max_scal[color%a_size];
            //printf("%d %d\n", no_of_cpu_allocated, mpitask_id);

            if(mpitask_id < no_of_cpu_allocated)
            {
                break;
            }

            if((mpitask_id >= table.total_max_scal) || (color > a_size))
            {
                color = mpitask_id % (a_size);
                break;
            }
...
//        T_MIN  = table.temperature[color/a_size];
//        T_MAX  = T_MIN;
//        T_SIZE = 1;
        A_0_MIN  = table.a_no[color%a_size];
        A_0_MAX  = A_0_MIN;
        A_0_SIZE = 1;

        MPI_Comm_split(MPI_COMM_WORLD,color,mpitask_id,&new_comm);

        int new_rank,new_size, MAX_SIZE;

        MPI_Comm_rank(new_comm, &new_rank);
        MPI_Comm_size(new_comm, &new_size);

        //printf("%d\t%d\t%d\n",new_rank,new_size,color);

        MAX_SIZE = nb_size*nmz_size*a_size*t_size;
        if(new_rank ==0)
        {
            manager(new_size-1, NMZ_MIN , NMZ_MAX , NB_MIN , NB_MAX , T_MIN , T_MAX, T_DISP, A_0_DISP, NB_DISP, NMZ_DISP, A_0_MIN, A_0_MAX); ///Arguments and other stuff needs to be added
        }
        else
        {
            int job[4],i,NMZ,NB,T,A_0;
            ...
            worker(new_rank,MAX_SIZE,job);///Arguments and other stuff needs to be added
        }
    }
    else
    {
        ///Use the scheduler to schedule the groups among the workers
       ///Call the master() and slave(). Master will tell the worker to make a new group when sufficient space is available

      /*  data3 new_table = table;

        knapnsort(&table,&new_table,a_size,number_of_processors);
*/
       if(mpitask_id == 0) master(&table, a_size);
       else slave(&table, NMZ_MIN , NMZ_MAX , NB_MIN , NB_MAX , T_MIN , T_MAX, T_DISP,A_0_DISP,NB_DISP, NMZ_DISP, A_0_MIN, A_0_MAX);
    }
    MPI_Finalize();
    return 0;
}
...
void manager(int number_of_processors,int NMZ_MIN ,int NMZ_MAX ,int NB_MIN ,int NB_MAX ,int T_MIN ,int T_MAX,int T_DISP,int A_0_DISP, int NB_DISP, int NMZ_DISP, int A_0_MIN,int A_0_MAX)
{
    ...
    int a_size,t_size,nb_size, nmz_size;
    ...
    long int MAX_SIZE = nb_size*nmz_size*a_size*t_size;
    ...
    */
    ...
    MPI_Pack_size( 4, MPI_INT, new_comm, &s1 );
    bufsize = (number_of_processors+10) * MPI_BSEND_OVERHEAD + s1*(number_of_processors+10);
    ...
    while (i<MAX_SIZE)
    {
        ...
        if (!((NMZ<A_0) && (NMZ> (-1)*A_0))) ///Checks if the values of NMZ and A_0 are valid.
        {
            i++;
            continue;
        }
        else
        {
            ...
            MPI_Recv(&temp, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, new_comm, &status); ///Receives the confirmation of task completeion from the worker().
            who = status.MPI_SOURCE;
            tag = status.MPI_TAG;
            if(s1==number_of_processors)
            {
                s1=0;
            ...
            MPI_Bsend(&job, 4, MPI_INT, who, 1, new_comm); ///Sends the new job to the worker().
            s1++;
            i++;
        }
    }
    ...
    for (i=0; i<number_of_processors; i++)
    {
        ...
        who = status.MPI_SOURCE;
        tag = status.MPI_TAG;
        MPI_Bsend(&job, 4, MPI_INT, who, 0, new_comm); ///Sends the message to the worker() to Stop/Finalize.
    }
    ...
    writecount = sprintf(timefile,"\n%d\t%lf\t%lf\t%lf\t",A_0,stime,ftime,(ftime-stime));

    MPI_File_open(MPI_COMM_SELF,"times.txt",(MPI_MODE_WRONLY | MPI_MODE_CREATE),MPI_INFO_NULL,&fh);

    MPI_File_write_at(fh,A_0*100,timefile,writecount,MPI_CHAR,MPI_STATUS_IGNORE);
...
void worker (int mpitask_id, long int MAX_SIZE, int task[])
{
    ...
    MPI_Comm_size(new_comm, &number_of_processors);
...
//        mkdir(foldername,S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    ...
    if (!((NMZ<A_0) && (NMZ > (-1)*A_0))) ///Checks if the values of NMZ and A_0 are valid
    {
        MPI_Send(&result, 1, MPI_INT, 0, 1, new_comm);
        MPI_Recv(&job, 4, MPI_INT, 0, MPI_ANY_TAG, new_comm,&status);
        tag = status.MPI_TAG;
    }
    else tag = mpitask_id;
...
*/
    ...
    while (tag) ///if tag == 0, no more work, else do work
    {
        A_0 = job[3];
        T = job[2];
        NB = job[1];
        ...
        offset += (number_of_processors-1)*sizeof(char)*35;
        ...
        /*
                ...
                fclose(sakfile);
        */
        if(count >= WRITE_BLOCK_SIZE) ///Write data stored in the 'point'
        {
            for(loopcount=0; loopcount<count; loopcount++)
            {
                ...
                /*    MPI_File_open(MPI_COMM_SELF,nucfile,(MPI_MODE_WRONLY | MPI_MODE_CREATE ), MPI_INFO_NULL, &fh);

                    MPI_File_write_at( fh, offset_t ,tstring, write_count, MPI_CHAR, MPI_STATUS_IGNORE);

                    MPI_File_close( &fh );

                    offset_t += sizeof(char)*140;
                */
            ...
            point=(data2*)malloc(sizeof(data2));
*/
            //offset_t += (number_of_processors)*sizeof(char)*140*174;
        ...
        MPI_Send(&result, 1, MPI_INT, 0, tag, new_comm);
        MPI_Recv(&job, 4, MPI_INT, 0, MPI_ANY_TAG, new_comm,&status);
        tag = status.MPI_TAG;
//MPI_Wait(&req, MPI_STATUS_IGNORE);
    ...
    write_count = sprintf(time, "\n%d\t%d\t%lf\t%lf\t%lf\t",old_mpitask_id, mpitask_id,finaltime, initialtime, initime);

    MPI_File_open(MPI_COMM_SELF,totaltimefile,(MPI_MODE_WRONLY | MPI_MODE_CREATE | MPI_MODE_APPEND), MPI_INFO_NULL, &fh1);

    MPI_File_write_at( fh1, sizeof(char)*30*(mpitask_id-1),time ,write_count, MPI_CHAR, MPI_STATUS_IGNORE);

    //MPI_File_write_at(fh,offset,time,sizeof(char)*50,MPI_CHAR,MPI_STATUS_IGNORE);
    MPI_File_close(&fh1);
...
void master(data3 * table, int a_size)
{
    ...
    MPI_Pack_size( (10000*(no_of_procs+10)), MPI_INT, MPI_COMM_WORLD, &s1 );
    bufsize = (no_of_procs+10) * MPI_BSEND_OVERHEAD + s1*(no_of_procs+10);
    buf = (double *)malloc( bufsize );
    MPI_Buffer_attach( buf, bufsize  );
*/
    int i,j,temp,color =0,old_color,k=1,who,no_of_cpu_allocated = 0;
    old_color = color;
    no_of_cpu_allocated += table->max_scal[color];

    for(j=1;j<no_of_procs;j++)
    {
        if(j > no_of_cpu_allocated)
        {
            old_color = color;
            //printf("%d\t%d\t%d\n",data_ranks[0],data_ranks[0]/a_size,data_ranks[0]%a_size);
            color++;
            no_of_cpu_allocated += table->max_scal[color];
        }
        if(color == old_color)
        ...
        else
        {
            ...
            for(m = 1;m<k;m++)
            {
                MPI_Send(data_ranks,k,MPI_INT,data_ranks[m],1,MPI_COMM_WORLD); ///Sends the color and the list of free slaves to the slaves.
...
*/
    while(1)
    {
        ...
        MPI_Recv(&temp,1,MPI_INT,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&status); ///Receives the notification of worker complete from the slave().
        ...
        if(color == old_color)
        {
            data_ranks[k++] = who;  ///Saves the rank of the slvae in the list.
        }
        else
        {
           for(m = 1;m<k;m++)
            ...
            if(color == a_size)
            {
                MPI_Send(&color,1,MPI_INT,who,0,MPI_COMM_WORLD); ///Sends message to the  slave that no more work left
                //printf("Break Here\n");
                break;
            }
            old_color = color;
...
void slave(data3 *table,int NMZ_MIN , int NMZ_MAX , int NB_MIN , int NB_MAX , int T_MIN , int T_MAX, int T_DISP,int A_0_DISP, int NB_DISP, int NMZ_DISP, int A_0_MIN, int A_0_MAX)
{
    int data_ranks[2000],tag,probe_count, result=0, color,A_0_SIZE = A_0_MAX-A_0_MIN+1;
    MPI_Status status;

    int NMZ_SIZE = NMZ_MAX-NMZ_MIN+1;
    int NB_SIZE = NB_MAX-NB_MIN+1;
    int T_SIZE = T_MAX-T_MIN+1;
    int a_size,t_size,nb_size, nmz_size;
    t_size = ceil(T_SIZE/T_DISP);
     nb_size = ceil(NB_SIZE/NB_DISP);
    nmz_size = ceil(NMZ_SIZE/NMZ_DISP);
    ...
    int mpitask_id, number_of_processors_in_mpi_world, new_rank, new_size;

        int group_rank, group_size, merge_size,group_id,*pids;
        pids  = &data_ranks[1];

    MPI_Comm_rank(MPI_COMM_WORLD,&mpitask_id);
    MPI_Probe(0,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
    MPI_Get_count(&status,MPI_INT,&probe_count);

    MPI_Recv(data_ranks,probe_count,MPI_INT,0,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
    tag = status.MPI_TAG;
    int i;

    /** Communicators are created with the help from:
James Dinan , Sriram Krishnamoorthy , Pavan Balaji , Jeff R. Hammond , Manojkumar Krishnan , Vinod Tipparaju , Abhinav Vishnu, Noncollective communicator creation in MPI, Proceedings of the 18th European MPI Users' Group conference on Recent advances in the message passing interface, p.282-291, September 18-21, 2011, Santorini, Greece **/


    while(tag) ///If tag == 0, then stop/Finalize, else do work.
    {
        //printf("%d\n",data_ranks[0]);
        ...
        {
            printf("**Slave:\t%d\t%d\n",mpitask_id,data_ranks[i]);
        }*/

        /**Create new group and communicator for the job with the help*/

        MPI_Comm ic;

        MPI_Comm_group(MPI_COMM_WORLD, &orig_group);

        MPI_Group_incl(orig_group,probe_count-1,pids,&new_group);

        MPI_Group_rank(new_group,&group_rank);
        MPI_Group_size(new_group,&group_size);
        MPI_Comm_dup(MPI_COMM_SELF,&new_comm);

        //MPI_Group_translate_ranks(new_group,group_size,&data_ranks[1],orig_group,pids);

        for(merge_size = 1; merge_size<group_size; merge_size=merge_size*2)
        {
            group_id = group_rank/merge_size;
            old_comm = new_comm;
            if(group_id %2 ==0)
            {
                if((group_id+1)*merge_size <group_size)
                {
                    MPI_Intercomm_create(new_comm,0,MPI_COMM_WORLD,pids[(group_id+1)*merge_size],data_ranks[0],&ic);
      ...
      //  printf("%d\t%d\t%d\t%d\n",data_ranks[0],mpitask_id, new_rank, new_size);
      ...
      /*  T_MIN  = table->temperature[color/a_size];
        T_MAX  = T_MIN;
        T_SIZE = 1;*/
        /**Start the manager and the workers*/

        A_0_MIN  = table->a_no[color];
        A_0_MAX  = A_0_MIN;
        A_0_SIZE = 1;
        a_size = 1;
        ...
        if(new_rank ==0)
        {
            //printf("%d\t%d\t%d\t%d\n",data_ranks[0],mpitask_id, new_rank, new_size);
            //printf("%d\t%d\n",T_MAX,A_0_MAX);
            manager( new_size-1, NMZ_MIN , NMZ_MAX , NB_MIN , NB_MAX , T_MIN , T_MAX, T_DISP,A_0_DISP,NB_DISP,NMZ_DISP, A_0_MIN, A_0_MAX);
        }
        else
        {
            int job[4],i,NMZ,NB,T,A_0;
